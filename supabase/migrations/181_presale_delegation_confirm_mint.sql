-- Debit the delegated source wallet's presale credits when the mint wallet confirms a presale mint.

CREATE OR REPLACE FUNCTION public.resolve_presale_credit_wallet(p_wallet text)
RETURNS text
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  v_source text;
BEGIN
  SELECT source_wallet INTO v_source
  FROM public.gen2_presale_mint_delegations
  WHERE mint_wallet = p_wallet;
...
REVOKE ALL ON FUNCTION public.resolve_presale_credit_wallet(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_presale_credit_wallet(text) TO service_role;
