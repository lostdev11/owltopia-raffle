-- One free ticket checkout via referral (amount_paid = 0); confirmed with token + RPC (no chain payment).
...
CREATE INDEX IF NOT EXISTS idx_entries_complimentary_pending
  ON public.entries (raffle_id, wallet_address)
  WHERE referral_complimentary = true AND status = 'pending';

COMMENT ON COLUMN public.entries.referral_complimentary IS
  'True when this row is a referral promotional free ticket (amount_paid 0); confirm via confirm_complimentary_referral_entry.';
...
COMMENT ON FUNCTION public.confirm_complimentary_referral_entry(UUID, TEXT) IS
  'Atomically confirm a referral complimentary entry (0 paid) using a one-time server token.';
