-- Presale overshoot (13 spots): separate mint phase + per-wallet allocations (admin-assigned).

ALTER TABLE public.owl_center_launches
  ADD COLUMN IF NOT EXISTS presale_overage_supply int NOT NULL DEFAULT 13;

COMMENT ON COLUMN public.owl_center_launches.presale_overage_supply IS
  'Spots for PRESALE_OVERAGE phase when presale sold past 657 (e.g. 670 sold, 13 extra).';

CREATE TABLE IF NOT EXISTS public.gen2_presale_overage_allocations (
  wallet text PRIMARY KEY,
  allowed_mints int NOT NULL DEFAULT 0,
  used_mints int NOT NULL DEFAULT 0,
  note text,
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT gen2_presale_overage_allowed_nonneg CHECK (allowed_mints >= 0),
  CONSTRAINT gen2_presale_overage_used_nonneg CHECK (used_mints >= 0),
  CONSTRAINT gen2_presale_overage_used_cap CHECK (used_mints <= allowed_mints)
);

COMMENT ON TABLE public.gen2_presale_overage_allocations IS
  'Wallets eligible for PRESALE_OVERAGE phase (13 overshoot spots); admin-managed.';

ALTER TABLE public.gen2_presale_overage_allocations ENABLE ROW LEVEL SECURITY;

GRANT SELECT, INSERT, UPDATE, DELETE ON public.gen2_presale_overage_allocations TO service_role;

ALTER TABLE public.owl_center_mint_events DROP CONSTRAINT IF EXISTS owl_center_mint_events_phase_check;
ALTER TABLE public.owl_center_mint_events
  ADD CONSTRAINT owl_center_mint_events_phase_check CHECK (
    phase IN (
      'AIRDROP',
      'PRESALE',
      'PRESALE_OVERAGE',
...
-- Allow PRESALE_OVERAGE as active phase (drop/recreate launch phase check if present).
ALTER TABLE public.owl_center_launches DROP CONSTRAINT IF EXISTS owl_center_launches_active_phase_check;
ALTER TABLE public.owl_center_launches DROP CONSTRAINT IF EXISTS owl_center_launches_phase_check;
ALTER TABLE public.owl_center_launches
  ADD CONSTRAINT owl_center_launches_phase_check CHECK (
    active_phase IN (
      'AIRDROP',
      'PRESALE',
      'PRESALE_OVERAGE',
