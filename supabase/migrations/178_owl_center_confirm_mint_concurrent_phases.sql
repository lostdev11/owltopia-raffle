-- Allow confirm_owl_center_gen2_mint to accept mints in ANY concurrently-live phase.
--
-- Previously the RPC accepted p_phase only when it equalled active_phase (plus the special-cased
-- Gen1 7-day airdrop window from migration 176). With admin-controlled concurrent phases
-- (`active_phases`, migration 177), a wallet can legitimately mint in any phase that is currently
-- live. This recreates the function so the phase gate accepts:
--   * p_phase = active_phase                                  (primary phase, as before)
--   * p_phase listed in active_phases                         (admin-toggled concurrent phase)
--   * p_phase = AIRDROP within the Gen1 7-day window          (as migration 176)
--
-- All per-phase supply/allocation caps below are unchanged — they remain the real spend ceilings.
...
BEGIN
  ...
  -- Phase gate: the requested phase must be LIVE. Live = the primary active_phase, OR any phase
  -- listed in active_phases (admin-toggled concurrent phases), OR AIRDROP within the Gen1 7-day
  -- holder window (mirrors the on-chain `gen1` group which stays open for 7 days).
  v_phase_live := (
    p_phase = v_launch.active_phase
    OR p_phase IN (
      SELECT jsonb_array_elements_text(coalesce(v_launch.active_phases, '[]'::jsonb))
    )
  );

  IF NOT v_phase_live AND p_phase = 'AIRDROP' THEN
    v_gen1_start := coalesce(
      nullif(v_launch.phase_schedule->>'AIRDROP', '')::timestamptz,
      v_launch.launch_deadline_at
    );
    IF v_gen1_start IS NOT NULL
       AND now() >= v_gen1_start
       AND now() <= v_gen1_start + interval '7 days' THEN
      v_phase_live := true;
    END IF;
  END IF;

  IF NOT v_phase_live THEN
  ...
  ELSIF p_phase = 'WHITELIST' THEN
    -- Global WL pool cap (atomic, under the advisory lock) — restored from migration 136 so
    -- concurrent confirm requests at the pool boundary cannot collectively exceed wl_supply.
