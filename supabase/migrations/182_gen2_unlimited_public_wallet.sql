-- Gen2 public phase: no per-wallet cap (unlimited within the public pool + total supply).
-- Off-chain eligibility uses gen2PublicWalletLimitRemaining; this removes the DB gate for slug=gen2.
...
COMMENT ON FUNCTION public.confirm_owl_center_gen2_mint(text, text, text, int, text, text[], text, text) IS
  'Service-role only: record mint, bump supply, presale/WL debits; Gen2 PUBLIC has no per-wallet cap.';
