-- Owl Nesting: atomic reward-claim ledger (position update + staking_reward_events row).
-- Idempotent when the same Solana transaction signature is replayed.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'staking_reward_events_execution_path_check'
  ) THEN
    ALTER TABLE public.staking_reward_events
      ADD CONSTRAINT staking_reward_events_execution_path_check
      CHECK (
        execution_path IS NULL
        OR execution_path IN ('onchain_transfer', 'database_only')
      );
  END IF;
END $$;

COMMENT ON COLUMN public.staking_reward_events.execution_path IS
  'onchain_transfer = SPL sent; database_only = app ledger only (e.g. NESTING_ALLOW_DB_ONLY_OWL_CLAIMS).';

CREATE UNIQUE INDEX IF NOT EXISTS idx_staking_reward_events_tx_sig_unique
  ON public.staking_reward_events (transaction_signature)
  ...
  IF v_sig IS NOT NULL THEN
    ...
    IF FOUND THEN
      IF v_existing.position_id IS DISTINCT FROM p_position_id
         OR v_existing.wallet_address IS DISTINCT FROM btrim(p_wallet)
...
COMMENT ON FUNCTION public.staking_record_reward_claim IS
  'Atomically bumps staking_positions.claimed_rewards and appends a claim row to staking_reward_events.';

GRANT EXECUTE ON FUNCTION public.staking_record_reward_claim(
  UUID, TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT
) TO service_role;

-- Backfill execution_path on historical claim rows.
UPDATE public.staking_reward_events
SET execution_path = 'onchain_transfer'
WHERE event_type = 'claim'
  AND execution_path IS NULL
  AND transaction_signature IS NOT NULL
  AND btrim(transaction_signature) <> '';

UPDATE public.staking_reward_events
SET execution_path = 'database_only'
WHERE event_type = 'claim'
  AND execution_path IS NULL
  AND NULLIF(btrim(COALESCE(note, '')), '') = 'db_only_owl_claim';
