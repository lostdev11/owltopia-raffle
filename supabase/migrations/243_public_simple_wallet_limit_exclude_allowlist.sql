-- Partner public_simple: wallet_mint_limit is per phase and must stack.
-- Allowlist-window confirms are stored as phase=PUBLIC in mint_events while soft WL usage
-- lives on owl_center_launch_wl_wallets.used_mints. Subtract allowlist usage so WL 2 + Public 5
-- allows 7 total instead of incorrectly blocking at 5.
...
DECLARE
  ...
  v_allowlist_used int;
  v_network text;
  v_cm text;
  v_gen1_start timestamptz;
  v_phase_live boolean;
  v_presale_credit_wallet text;
BEGIN
  ...
  v_presale_credit_wallet := public.resolve_presale_credit_wallet(p_wallet);
  ...
  IF p_phase = 'PRESALE' THEN
    ...
    END IF;
    PERFORM public.use_gen2_presale_mints(v_presale_credit_wallet, p_quantity);
  ...
  ELSIF p_phase = 'WHITELIST' THEN
    ...
    END IF;
  ELSIF p_phase = 'PUBLIC' AND p_launch_slug <> 'gen2' THEN
    SELECT COALESCE(SUM(quantity), 0)::int INTO v_public_minted
    FROM owl_center_mint_events
    ...
    -- public_simple stores allowlist-window mints as PUBLIC too; exclude soft WL usage.
    IF v_launch.mint_mode = 'public_simple' THEN
      SELECT COALESCE(SUM(used_mints), 0)::int INTO v_allowlist_used
      FROM owl_center_launch_wl_wallets
      WHERE launch_id = v_launch_id
        AND wallet = p_wallet;
      v_public_minted := GREATEST(0, v_public_minted - COALESCE(v_allowlist_used, 0));
    END IF;
...
COMMENT ON FUNCTION public.confirm_owl_center_gen2_mint(text, text, text, int, text, text[], text, text) IS
  'Service-role only: record mint, bump supply, presale/WL debits; Gen2 PUBLIC has no per-wallet cap; public_simple public wallet limit excludes allowlist used_mints.';
