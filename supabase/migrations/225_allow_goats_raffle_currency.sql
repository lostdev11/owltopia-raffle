-- Catch-up: GOATS ticket currency was added in app code and 212_goats_partner_pro_token.sql,
-- but 212 never landed on production. Create failed with raffles_currency_check
-- (allowed USDC/SOL/OWL/BAMBOO only). Ticket price 57,800 GOATS is fine after
-- 216 / 20260814001333 widened ticket_price.
...
-- List/carousel poll RPC: add GOATS revenue (CREATE OR REPLACE cannot add OUT columns).
DROP FUNCTION IF EXISTS public.summarize_entries_for_raffle_ids(uuid[], text);

CREATE FUNCTION public.summarize_entries_for_raffle_ids(
  p_ids uuid[],
