  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            'Force-cancel is only for milestone raffles with no creator cancellation request and no winner yet (draft, live, ready_to_draw, or pending_min_not_met). Use Accept cancellation if the creator already requested cancel, or return milestone deposits if already cancelled/failed-refund.',
        },
        { status: 400 }
      )
    }
