...
A cosmetic animation that displays when users select ticket quantities. The meter animates based on the number of tickets selected. **Winner selection** uses each **confirmed ticket** as one draw chance (more tickets ⇒ higher win probability); the Hoot Boost meter is still **cosmetic** and does not apply a separate multiplier to the draw. This is for visual engagement only.
