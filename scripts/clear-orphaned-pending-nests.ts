 ...
 * Usage: npx --yes tsx scripts/clear-orphaned-pending-nests.ts <wallet> [wallet2...]
 */
...
if (wallets.length === 0) {
  console.error('Usage: npx --yes tsx scripts/clear-orphaned-pending-nests.ts <wallet> ...')
  process.exit(1)
}

async function main() {
  for (const wallet of wallets) {
    const result = await clearOrphanedPendingNftNestsForWallet(wallet.trim())
