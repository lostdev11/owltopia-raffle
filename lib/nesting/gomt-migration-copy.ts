...
export const GOMT_RESTAKE_REQUIRED_IF_CONTINUING =
  'To nest on Owltopia, unstake on GOTM Labz first (or wait until your GOTM lock ends). Owltopia nests start fresh — new lock timer and new rewards. Prior GOTM staking time does not carry over.'

export const GEN1_UNLOCK_TIMING =
  'Unlock dates are per holder, based on when you originally staked. Early GOTM staking started in January with 180-day locks, so the first unlocks roll through over time — not one shared date for everyone.'
...
export const GOMT_MIGRATION_FAQ: readonly { q: string; a: string }[] = [
  {
    q: 'Do I have to move from GOTM Labz to Owltopia?',
