...
export function buildAdminSupportPlaybook(params: {
  ...
  if (significantUnpaid && onchain24hTx === 0) {
    recommendations.push({
      ...
      detail:
        'If they paid the SOL platform fee but no OWL left the reward treasury, claimable OWL stays on the nests. The on-chain tx is usually a plain SOL transfer to the platform fee treasury (System Program only) — not an OWL SPL transfer. Retry Claim all — the app reuses a paid fee signature (localStorage, GET /api/me/staking/claim-all/preview `reusable_platform_fee_signature`, or server recovery within 48h). Claim all now runs lock checks in preview before charging the fee. Claim all skips only nests that are truly unlocked (Finish opening); transient RPC lock-read failures abort the whole Claim all with no OWL sent so the fee can be reused — they must not silently underpay. Do not refund SOL until retry either succeeds or confirms a second fee was charged. Check reward treasury OWL + SOL funding if transfers keep failing.',
    })
  }
  ...
  if (genActive > 0) {
    recommendations.push({
      ...
      detail:
        'Daily OWL claims use Claim all / per-nest claim. Month-end SOL/USDC rev share is separate (Gen Owl rev share panel) and pays from the dedicated rev-share pool — never funds escrow, and never from an admin personal wallet alone. Do not use OWL catch-up for rev-share payouts. Platform fees for rev-share claims go to the mint-fee treasury and do not fund the pool. Outstanding liability = deposited − reserved/paid claims across every open month. Claim rows are reserved before any pool payout; they are only deleted on clear pre-send failures — never after a tx may have landed. Every successful pool send is also written to gen_owl_rev_share_pool_payouts; coverage subtracts orphan ledger outflows so a missing claim row cannot reopen the shortfall gate. If a payout hit the chain without claim rows, Admin can POST /api/admin/gen-owl-rev-share/record-pool-payout with the signature. If users see "Waiting for pool top-up", use Admin → Cover shortfall (sends SOL to the dedicated pool without raising claimable books). Do not use Deposit Gen 1/2 to close a coverage hole — that credits period totals and the shortfall stays. Finalize now freezes each nest’s standard/1/1 bucket so claim-time DAS cannot overpay 1/1 bonuses. Paid-but-failed rev-share fees are reusable for 48h (same as OWL claim fee recovery).',
    })
  }
  ...
  if (activeUnderWallet === 0 && diag.wallet_nest_mint_count > 0) {
    ...
    recommendations.push({
      ...
      detail:
        'Four Ledger pain points: (1) Nest "Say hi" — Phantom/Solflare often never deliver off-chain Sign Message to Ledger. Use the My nest button “Sign with Ledger transaction” (memo tx, not broadcast). Unlock device, Solana app open, Ledger Live closed; USB > Bluetooth. (2) Nesting notes box — same Sign Message gap (Phantom “Sign message rejected” / error Code 1). Use “Sign with Ledger” in the nesting notes box (memo tx, not broadcast); auto-falls back after Sign Message failure. (3) Nest lock "Unexpected instruction" / Phantom-Lighthouse string — Solflare/Phantom inject Lighthouse guards Ledger cannot clear-sign; Gainz-style stakes can still work — nest from a hot wallet or transfer then nest. (4) Nest lock never prompts — complex Metaplex Core versioned txs; nest one NFT at a time or from a hot wallet. (5) Dashboard / ticket refunds “Invalid signature” — same Sign Message gap. On My Dashboard use “Sign with Ledger transaction”; on raffle Claim refund, retry after unlock — auto-falls back to memo-tx. Refunds are paid by funds escrow (buyer does not need SOL for the payout fee).',
    })
  }
