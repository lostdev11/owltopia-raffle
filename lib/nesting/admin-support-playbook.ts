...
export function buildAdminSupportPlaybook(params: {
  ...
  if (genActive > 0) {
    recommendations.push({
      ...
      detail:
        'Daily OWL claims use Claim all / per-nest claim. Month-end SOL/USDC rev share is separate (Gen Owl rev share panel) and pays from the dedicated rev-share pool — never funds escrow. Do not use OWL catch-up for rev-share payouts. Platform fees for rev-share claims go to the mint-fee treasury and do not fund the pool. Outstanding liability = deposited − paid across every open month (claimed vs unclaimed nests). Admin deposit panel shows that stack; claims refuse until the on-chain pool covers remaining unclaimed SOL so late claimers are not stranded. If users see "Waiting for pool top-up", deposit the shortfall (Admin → Gen Owl deposit). Finalize now freezes each nest’s standard/1/1 bucket so claim-time DAS cannot overpay 1/1 bonuses. Paid-but-failed rev-share fees are reusable for 48h (same as OWL claim fee recovery).',
    })
  }
