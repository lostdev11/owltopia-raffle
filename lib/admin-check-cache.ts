...
type CacheEntry = { wallet: string; isAdmin: boolean; ts: number }
...
export function setCachedAdmin(wallet: string, isAdmin: boolean): void {
  if (typeof window === 'undefined' || !wallet) return
  cached = { wallet, isAdmin, ts: Date.now() }
}
