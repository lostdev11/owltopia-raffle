...
type AccessState = {
  /** True when viewer may use OwlSend (public flag, SIWS admin session, or connected admin wallet). */
  allowed: boolean
...
/**
 * OwlSend access gate aligned with Header:
 ...
 */
export function useOwlSendAdminAccess(params: {
  ...
  // SIWS session — same pattern as Header (survives disconnect).
  useEffect(() => {
    if (isPublic) return
    let cancelled = false
