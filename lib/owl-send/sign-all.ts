...
export type OwlSendTokenSignAllBuilt = {
  tx: Transaction
  lines: import('@/lib/owl-send/send-tokens').OwlSendTokenLine[]
  newAtaCount: number
}
...
/**
 * One wallet sheet for a window of token-scatter approvals.
 * Same transport as NFT sign-all; lines are token transfer rows for ledger recording.
 */
export async function sendOwlSendSignedTokenBatchGroup(params: {
  connection: Connection
  owner: PublicKey
  walletAdapter: WalletAdapter | null
  built: OwlSendTokenSignAllBuilt[]
  onPhase?: (phase: 'approving' | 'confirming') => void
}): Promise<Array<{ ok: true; signature: string; newAtaCount: number }>> {
  const { connection, owner, walletAdapter, built } = params
  if (built.length < 1) return []
  ...
  params.onPhase?.('confirming')
  const out: Array<{ ok: true; signature: string; newAtaCount: number }> = []
  for (let i = 0; i < built.length; i++) {
    const signature = signatures[i]
    if (!signature) {
      throw new Error(`Wallet did not return a signature for approval ${i + 1} of ${built.length}.`)
